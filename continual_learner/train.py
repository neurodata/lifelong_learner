...
def shape_change_jd(data):
    data_len = data.shape[0]
    
    data_ = torch.zeros([data_len, 3, 32, 32]) #np.zeros((data_len,3,32,32),dtype = float)
    #print(data_.shape,'hi', data.shape)
    for i in range(data_len):
        data_[i,0,:,:] = data[i,:,0:32,0:32]
        data_[i,1,:,:] = data[i,:,0:32,32:64]
        data_[i,2,:,:] = data[i,:,32:64,0:32]
        #print(data_[i,:,:,:])
        #a = input()
    return data_

#tasks_to_complete, name JD param
def train_cl(tasks_to_complete, name, model, train_datasets, test_datasets, replay_mode="none", scenario="class",classes_per_task=None,iters=2000,batch_size=32,
    ...
    for task, train_dataset in enumerate(train_datasets, 1):
        ...
        average_precs = sum(precs) / 10
        print('=> average precision over all {} tasks: {:.4f}'.format(10, average_precs))
        ...
        with open('crossval_res/'+name+str(task)+'__'+str(cv)+'.pickle', 'wb') as f:
            pickle.dump(precs, f)
        ...
        #single task error code
        print(task,'---> Task')
        #`1print(test_datasets)
        precs = [evaluate.validate(
        model, test_datasets[i], verbose=False, test_size=None, task=i+1, with_exemplars=False,
        allowed_classes=list(range(classes_per_task*i, classes_per_task*(i+1))) if scenario=="task" else None
        ) for i in range(1)]
        ...
        for i in range(1):
            precs[i] = 1 - precs[i]
            print(" - Task {}: {:.4f}".format(i + 1, precs[i]))
        average_precs = sum(precs) / 1
        print('=> average precision over all {} tasks: {:.4f}'.format(10, average_precs))
        
        with open('task_count.pickle', 'rb') as f:
            task_count = pickle.load(f)
        ...
        with open('crossval_test/single_task'+str(t)+'__'+str(cv)+'.pickle', 'wb') as f:
            pickle.dump(precs, f)
        ...
        if task == tasks_to_complete:
            ...
            task_count[name].pop(0) 
            with open('task_count.pickle', 'wb') as f:
                pickle.dump(task_count,f)
            
            sys.exit()
            
        #a = input()'''
    
        print(task,tasks_to_complete,'---> Task')
        ...
        with open('./'+name+'/task'+str(task)+'.pickle', 'wb') as f:
            pickle.dump(precs, f)
